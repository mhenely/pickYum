...
- **Deployment**: backend has a Dockerfile in `server/`; CD workflow at `.github/workflows/deploy.yml` builds the image to GHCR on every CI-passing main push and supports staging auto-deploy + manual production promotion.
